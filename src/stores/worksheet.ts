...
import { cache } from '@/shared/utils/storeCache'
...
export const useWorksheetStore = cache((name: string) =>
  defineStore(`worksheet${name}`, () => {
    ...
    const addParallel = () => {
      if (!worksheetIsLocked.value || !selectedMethod.value || !selectedUser.value) return
      const parallel =
    ...
    const exportByURL = async (parallel: IParallel) => {
      if (!selectedMethod.value?.code || !selectedUser.value || !worksheetIsLocked.value) return
      try {
        const url = settingsStore.settings.export.url?.replace(
          /{([a-z][\w-]*(?:\.[\w-]+)*)}/g,
          (_, key: string) => {
            return getValueByPath(parallel, key) as string
          },
        ...
        if (url) {
          await exportAPI.exportByURL(url)
          parallel.exported_at = new Date()
        }
      } catch (e: unknown) {
        console.log(e)
      ...
      for (const line of data) {
        ...
        for (const idx in keys) {
          ...
          if (keys[idx] === 'exported_at') {
            _parallel[keys[idx]] = cells[idx] ? new Date(cells[idx]) : undefined
            continue
          }
          if (keys[idx] === 'result') {
